...
#define IMU_COMP_DT_S            0.005f

#ifndef IMU_PITCH_SIGN
#define IMU_PITCH_SIGN           (-1.0f)
#endif

static float pitch_filter_deg;
static uint8_t pitch_filter_ready;
...
{
  pitch_filter_deg = 0.0f;
  pitch_filter_ready = 0U;
}
...
uint8_t Imu_ReadRaw(ImuRaw_t *raw)
{
  ...
  if (i2cRead(MPU6050_ADDR, MPU6050_RA_ACCEL_XOUT_H, sizeof(buffer), buffer) == false)
  {
    return 0U;
  }
...
uint8_t Imu_UpdateComplementary5ms(BalanceSample_t *sample)
{
  ...
  accel_pitch_deg = (float)(atan2((double)raw.accel_y, (double)raw.accel_z) *
  ...
  if (pitch_filter_ready == 0U)
  {
    pitch_filter_deg = accel_pitch_deg;
    pitch_filter_ready = 1U;
  }
  else
  {
    pitch_filter_deg = (IMU_COMP_ALPHA *
                        (pitch_filter_deg + (gyro_pitch_dps * IMU_COMP_DT_S))) +
                       ((1.0f - IMU_COMP_ALPHA) * accel_pitch_deg);
  }

  sample->timestamp_ms = HAL_GetTick();
  sample->pitch_deg = pitch_filter_deg;
  sample->gyro_pitch_dps = gyro_pitch_dps;
  sample->raw = raw;
  sample->valid = 1U;
  return 1U;
}
